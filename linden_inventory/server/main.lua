...
AddEventHandler('linden_inventory:confiscatePlayerInventory', function(xPlayer, jobx)
	if type(xPlayer) ~= 'table' then xPlayer = ESX.GetPlayerFromId(xPlayer) end
	if Inventories[xPlayer.source] then
		local inventory = json.encode(getPlayerInventory(xPlayer, true))
		exports.ghmattimysql:execute('REPLACE INTO `linden_inventory` (owner, name, data) VALUES (@owner, @name, @data)', {
...
AddEventHandler('linden_inventory:recoverPlayerInventory', function(xPlayer, jobx)
	if type(xPlayer) ~= 'table' then xPlayer = ESX.GetPlayerFromId(xPlayer) end
	if Inventories[xPlayer.source] then
		local result = exports.ghmattimysql:scalarSync('SELECT data FROM `linden_inventory` WHERE owner =  @owner and  name = @name', {
			 ['@owner'] = xPlayer.getIdentifier(), 
			 ['@name'] = jobx,
			 })
		if result ~= nil then
			exports.ghmattimysql:execute('DELETE FROM `linden_inventory` WHERE owner =  @owner and name = @name', {
				 ['@owner'] = xPlayer.getIdentifier(),
				  ['@name'] = jobx,
				 })
