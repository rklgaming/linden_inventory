...
AddEventHandler('linden_inventory:water', function(item, wait, cb)
	...
	SetTimeout(wait, function()
		if not cancelled then
			...
			exports['okokNotify']:Alert("Drink", "You drank some refreshing water", 5000, 'success')
...
AddEventHandler('linden_inventory:bread', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You drank some refreshing water', length = 2500})
			exports['okokNotify']:Alert("Drink", "Eat  some Bread", 5000, 'success')
...
AddEventHandler('linden_inventory:horchate', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You drank some delicious horchate', length = 2500})
		end
	end)
end)
...
AddEventHandler('linden_inventory:blackcoffee', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Drink", "You Drink some blackcoffee", 5000, 'success')
		end
	end)
end)

AddEventHandler('linden_inventory:caramelfrappuccino', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Drink", "You Drink some caramelfrappuccino", 5000, 'success')
		end
	end)
end)

AddEventHandler('linden_inventory:icematchalatte', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Drink", "You Drink some icematchalatte", 5000, 'success')
		end
	end)
end)
...
AddEventHandler('linden_inventory:softtaco', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Eat", "You Eat some softtaco", 5000, 'success')
		end
	end)
end)

AddEventHandler('linden_inventory:burrito', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Eat", "You Eat some burrito", 5000, 'success')
		end
	end)
end)

AddEventHandler('linden_inventory:elote', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Eat", "You Eat some elote", 5000, 'success')
		end
	end)
end)

AddEventHandler('linden_inventory:doubledouble', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Eat", "You Eat some doubledouble", 5000, 'success')
		end
	end)
end)

AddEventHandler('linden_inventory:cheeseburger', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Eat", "You Eat some doubcheeseburgerledouble", 5000, 'success')
		end
	end)
end)

AddEventHandler('linden_inventory:frenchfries', function(item, wait, cb)
	cb(true)
	SetTimeout(wait, function()
		if not cancelled then
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'inform', text = 'You.. drank mustard', length = 2500})
			exports['okokNotify']:Alert("Eat", "You Eat some frenchfries", 5000, 'success')
		end
	end)
end)
