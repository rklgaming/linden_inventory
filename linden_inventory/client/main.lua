...
StartInventory = function()
	...
	SetTimeout(500, function()
		ESX.TriggerServerCallback('linden_inventory:setup', function(data)
			...
			print(msg)
			--TriggerEvent('mythic_notify:client:SendAlert', {type = 'info', text = msg, length = 2500})
			exports['okokNotify']:Alert("Inventory", msg, 2500, 'info')
	...
	if not invOpen and not IsPedInAnyVehicle(ESX.PlayerData.ped, true) and not currentInventory and CanOpenInventory() then
		...
		if targetPlayer ~= -1 and targetDistance <= 1.5 then
			if CanOpenTarget(GetPlayerPed(targetPlayer)) or ESX.PlayerData.job.name == 'police' then
				...
				exports['okokNotify']:Alert("Inventory", U('inventory_cannot_open_other'), 2500, 'error')
			end
		end
	end
end
...
AddEventHandler('linden_inventory:addAmmo', function(ammo)
	if currentWeapon and not isBusy then
		if currentWeapon.ammoname == ammo.name then
			...
			exports['okokNotify']:Alert("Inventory",  _U('wrong_ammo', currentWeapon.label, ammo.label), 2500, 'error')
		end
	end
end)
							...
							if IsControlJustPressed(1,38) then
								ESX.TriggerServerCallback('esx_license:checkLicense', function(hasWeaponLicense)
									if hasWeaponLicense then
										...
										exports['okokNotify']:Alert("Inventory",  _U('has_weapon_license'), 2500, 'error')
									else
										ESX.TriggerServerCallback('linden_inventory:buyLicense', function(bought)
											if bought then
												--TriggerEvent('mythic_notify:client:SendAlert', {type = 'info', text = _U('bought_weapon_license'), length = 2500})
												exports['okokNotify']:Alert("Inventory",  _U('bought_weapon_license'), 2500, 'info')
											else
												--TriggerEvent('mythic_notify:client:SendAlert', {type = 'info', text = _U('poor_weapon_license'), length = 2500})
												exports['okokNotify']:Alert("Inventory",  _U('poor_weapon_license'), 2500, 'info')
											end
										end, license)
									end
			...
			if currentInventory then
				if currentInventory.type == 'player' then
					...
					if not id or dist > 1.8 then
						...
						exports['okokNotify']:Alert("Inventory",  _U('inventory_lost_access'), 2500, 'error')
...
RegisterCommand('inv', function()
	if isBusy then --TriggerEvent('mythic_notify:client:SendAlert', {type = 'error', text = _U('inventory_cannot_open'), length = 2500})
		exports['okokNotify']:Alert("Inventory",  _U('inventory_cannot_open'), 2500, 'error')
...
RegisterCommand('inv2', function()
	...
	elseif currentInventory then TriggerEvent('linden_inventory:closeInventory')
		...
		if not CanOpenInventory() then
			-- TriggerEvent('mythic_notify:client:SendAlert', {type = 'error', text = _U('inventory_cannot_open'), length = 2500})
			exports['okokNotify']:Alert("Inventory",  _U('inventory_cannot_open'), 2500, 'error')
			return end
		if not IsPedInAnyVehicle(ESX.PlayerData.ped, false) then -- trunk
			...
			if vehicle and Config.Trunks[class] and #(playerCoords - vehiclePos) < 6 and NetworkGetEntityIsNetworked(vehicle) then
				...
				if locked == 0 or locked == 1 then
					...
					exports['okokNotify']:Alert("Inventory",  _U('vehicle_locked'), 2500, 'error')
				end
			end
...
RegisterCommand('weapondetails', function()
	if currentWeapon and ESX.PlayerData.job.name == 'police' then
		...
		--TriggerEvent('mythic_notify:client:SendAlert', {type = 'info', text = msg, length = 8000})
		exports['okokNotify']:Alert("Inventory",  msg, 2500, 'info')
	end
end)
...
RegisterNUICallback('notification', function(data)
	if data.type == 2 then data.type = 'error' else data.type = 'info' end
--	TriggerEvent('mythic_notify:client:SendAlert', {type = data.type, text = _U(data.message), length = 2500})
	exports['okokNotify']:Alert("Inventory",  _U(data.message), 2500, data.type)
end)
...
RegisterNUICallback('giveItem', function(data, cb)
	...
	if closestPlayer == -1 or closestPlayerDistance > 2.0 then 
		...
		exports['okokNotify']:Alert("Inventory",  _U('nobody_nearby'), 2500, 'error')
	elseif data.inv == 'Playerinv' then
		if data.amount >= 1 then
			TriggerServerEvent('linden_inventory:giveItem', data, GetPlayerServerId(closestPlayer))
			TriggerEvent('randPickupAnim')
		else
			 --TriggerEvent('mythic_notify:client:SendAlert', {type = 'error', text = _U('give_amount'), length = 2500})
			exports['okokNotify']:Alert("Inventory",  _U('give_amount'), 2500, 'error')
		end
	end
end)
...
RegisterNUICallback('BuyFromShop', function(data)
	if data.count >= 1 then
		...
		-- TriggerEvent('mythic_notify:client:SendAlert', {type = 'error', text = _U('buy_amount'), length = 2500})
		exports['okokNotify']:Alert("Inventory",  _U('buy_amount'), 2500, 'error')
	end
		...
		if data or esxItem then
			if data then
				if data.useinvehicle == false and IsPedInAnyVehicle(ESX.PlayerData.ped, true) then
					...
					exports['okokNotify']:Alert("Inventory",  _U('cannot_use', item.label), 2500, 'error')
				elseif data.component then
					...
					if not component then
						-- TriggerEvent('mythic_notify:client:SendAlert', {type = 'error', text = _U('component_invalid', item.label), length = 2500})
						exports['okokNotify']:Alert("Inventory", _U('component_invalid', item.label), 2500, 'error')
						return end
					if HasPedGotWeaponComponent(ESX.PlayerData.ped, currentWeapon.hash, component.hash) then
						--TriggerEvent('mythic_notify:client:SendAlert', {type = 'error', text = _U('component_has', item.label), length = 2500}) 
						exports['okokNotify']:Alert("Inventory", _U('component_has', item.label), 2500, 'error')
						return
					end
				end
			end
				...
				if data.event then
					...
					TriggerEvent(event, item, data.usetime, function(cb)
						if cb then
							...
							--TriggerEvent('mythic_notify:client:SendAlert', {type = 'error', text = _U('cannot_use', item.label), length = 2500}) 
							exports['okokNotify']:Alert("Inventory", _U('cannot_use', item.label), 2500, 'error')
							return
						end
					end)
